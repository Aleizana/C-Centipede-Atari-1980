...
const MoveState* TurnUpSwitchToRight::GetNextState(CentipedeHead* chead) const
{
	...
	{
		pNextState = (MoveState*)&MoveFSM::StateMoveRightEndUpwards;
	}
...
const MoveState* TurnUpSwitchToRight::GetNextState(int row, int col, sf::Vector2f pos) const
{
	const MoveState* pNextState;
...
const bool TurnUpSwitchToRight::CheckForPlrArea(CentipedeHead* headToMove) const
{
...
const bool TurnUpSwitchToRight::CheckForPlrArea(sf::Vector2f pos) const
{
	//Check if centipede head will be moving into the player area limit and return true if so
	//The player area limit is offset by the same value that the centipede head pos is offset at spawn
...
int TurnUpSwitchToRight::IncrementRow() const
{
	return RowIncrement;
}

int TurnUpSwitchToRight::IncrementCol() const
{
	return ColIncrement;
}

std::stack<int> TurnUpSwitchToRight::GetSpriteProperties(const int offsetArrIndex) const
{
