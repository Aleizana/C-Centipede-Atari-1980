...
{
public:
	//Enum variable across all GameObjects that determines whether they are 'dead', 'alive', or 'poisoned'
	enum LifeState { DEAD, ALIVE, POISON, DAMAGED, POISON_DMG };
	enum ObjectType {MUSHROOM, GLYPH};

protected:
	int health;

	//Variable to store the life state across any game object
	LifeState myState;

	//Variable to store the type of objects abstracted as GameObjects
	ObjectType myType;

private:
	//Universal damage taken and death HP for all game objects
	const int dmgAmount = 1;
	const int deathHP = 0;
...
public:
	//Getters for self-added variables
	int GetDmgAmount();
	int GetDeathHP();
	...
	virtual const int GetMyRow() const;
	virtual const int GetMyCol() const;

	virtual const int GetStartingHP() const;
