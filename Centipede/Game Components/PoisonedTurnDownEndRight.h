#ifndef _PoisonTurnDownEndRight
#define _PoisonTurnDownEndRight

#include "MoveState.h"

class PoisonedTurnDownEndRight : public MoveState
{
public:
	PoisonedTurnDownEndRight();
	PoisonedTurnDownEndRight(const PoisonedTurnDownEndRight& copyMe) = delete;
	PoisonedTurnDownEndRight& operator=(const PoisonedTurnDownEndRight&) = delete;
	~PoisonedTurnDownEndRight() = default;
...
private:
	...
	const float SpriteScalar_Y = 2.0f;	//For setting the size of the sprite, negative so as to flip the sprite on the x-axis to face it right
