...
MoveLeftEndUpwards::MoveLeftEndUpwards()
{
	std::cout << "Initializing MoveLeftEndUpwards state\n";

	MyMoveOffsets = MovementCollection::OffsetsStraightLeft;
}
void MoveLeftEndUpwards::Show() const
{
	std::cout << "MoveLeftEndUpwards";
}

const MoveState* MoveLeftEndUpwards::GetNextState(CentipedeHead* chead) const
{
	...
	GameGrid::CheckGridResult holdResult = GameManager::GetPlayerMgr()->GetActivePlayer()->GetPlayerGrid()->GameGrid::CheckGridSpot(holdRow, holdCol);

	
	//if there is nothing occupying the space
	if (holdResult == GameGrid::OPEN)
	{
		pNextState = &MoveFSM::StateMoveLeftEndUpwards;
	}
...
const MoveState* MoveLeftEndUpwards::GetNextState(int row, int col, sf::Vector2f pos) const
{
	const MoveState* pNextState;
...
const bool MoveLeftEndUpwards::CheckForPlrArea(CentipedeHead* headToMove) const
{
...
const bool MoveLeftEndUpwards::CheckForPlrArea(sf::Vector2f pos) const
{
	//Check if centipede head will be moving into the player area limit and return true if so
	//The player area limit is offset by the same value that the centipede head pos is offset at spawn
...
int MoveLeftEndUpwards::IncrementRow() const
{
	return RowIncrement;
}

int MoveLeftEndUpwards::IncrementCol() const
{
	return ColIncrement;
}

std::stack<int> MoveLeftEndUpwards::GetSpriteProperties(const int offsetArrIndex) const
{
