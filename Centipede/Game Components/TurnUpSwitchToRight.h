#ifndef _TurnUpSwitchRight
#define _TurnUpSwitchRight
...
public:
	TurnUpSwitchToRight();
	TurnUpSwitchToRight(const TurnUpSwitchToRight& copyMe) = delete;
	TurnUpSwitchToRight& operator=(const TurnUpSwitchToRight&) = delete;
	~TurnUpSwitchToRight() = default;
...
private:
	...
	const float SpriteScalar_Y_FirstState = -2.0f;	//For setting the size of the sprite, negative so as to flip the sprite on the y-axis to face it up
	const float SpriteScalar_X_FirstState = 2.0f;	//For setting the size of the sprite
